...
def kalman_filter_step(belief, control, measurement):
    ...
    mu_prediction = belief.mu+control.mu
    sigma2_prediction = belief.sigma2+control.sigma2
    prediction = Density(mu_prediction,sigma2_prediction)  # Replace
    #print(mu_prediction)

    # Correction.
    c=1
    k=(c*prediction.sigma2)/((c*c)*prediction.sigma2+measurement.sigma2)
    mu_correction=prediction.mu+k*(measurement.mu-c*prediction.mu)
    sigma2_correction=(1-k*c)*prediction.sigma2
    correction = Density(mu_correction,sigma2_correction)  # Replace
    #print(correction.mu)
    return (prediction, correction)
