...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,scanner_displacement):
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        motor_ticks_right=motor_ticks[1]*ticks_to_mm
        motor_ticks_left=motor_ticks[0]*ticks_to_mm
        alpha=(motor_ticks_right-motor_ticks_left)/robot_width
        radius=motor_ticks_left/alpha
        x_center=(old_pose[0]-scanner_displacement*math.cos(old_pose[2]))-(radius+robot_width/2)*math.sin(old_pose[2])
        y_center=(old_pose[1]-scanner_displacement*math.sin(old_pose[2]))+(radius+robot_width/2)*math.cos(old_pose[2])
        theta=(old_pose[2]+alpha)%(6.2832)
        x=x_center+(radius+robot_width/2)*math.sin(theta)+scanner_displacement*math.cos(theta)
        y=y_center-(radius+robot_width/2)*math.cos(theta)+scanner_displacement*math.sin(theta)
